...
void AttackSystem::update(entt::registry& registry, float deltaTime)
{
    ...
    attacker_view.each(
            [&registry, deltaTime](entt::entity ent, WeaponComponent& weapon, const position pos)
            {
    ...
    shooter_view.each(
            [&registry, deltaTime](entt::entity ent, ShootComponent& shooter, const position pos)
            {
